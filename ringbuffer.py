...
class RingBuffer():
    ...
    def push(self, floatdata):
        ...
        if dim != self.buffer.shape[0]:
            # switched from single to dual channels or vice vers
        ...
        self.buffer[:, offset + self.buffer_length: offset +
            self.buffer_length + direct] = floatdata[:, 0: direct]
    ...
    def data(self, length):
        ...
        if start > 2 * self.buffer_length or start < 0:
            raise ArithmeticError("Start index "
                            " is wrong %d %d" % (start, self.buffer_length))
        if stop > 2 * self.buffer_length:
            raise ArithmeticError("Stop index is larger than "
                    " buffer size: %d > %d" % (stop, 2 * self.buffer_length))
        return self.buffer[:, start: stop]

    def data_older(self, length, delay_samples):
        self.grow_if_needed(length + delay_samples)

        start = (self.offset - length - delay_samples) % (self.buffer_length +
                                                           self.buffer_length)
        stop = start + length
        return self.buffer[:, start: stop]
    ...
    def data_indexed(self, start, length):
        ...
        if start0 > 2 * self.buffer_length or start0 < 0:
            raise ArithmeticError("Start index is wrong %d %d"
                                  % (start0, self.buffer_length))
        if stop0 > 2 * self.buffer_length:
            raise ArithmeticError("Stop index is larger than buffer "
                    " size: %d > %d" % (stop0, 2 * self.buffer_length))
    ...
    def grow_if_needed(self, length):
        if length > self.buffer_length:
            ...
            message = ["Ringbuffer: growing buffer for"
                       " length %d" % (new_length)]
            if self.logger != None:
            ...
            # first copy, always complete
            newbuffer[:, shift:shift + old_length] = self.buffer[:,
                                                                 :old_length]
            # second copy, can be folded
            direct = min(old_length, new_length - shift)
            folded = old_length - direct
            newbuffer[:, new_length + shift:new_length +
                      shift + direct] = self.buffer[:, : direct]
