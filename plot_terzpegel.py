...
class PenStyles(QtGui.QWidget):
    ...
    def draw_text(self, painter):
        ...
        for i in range(0, len(self.freqValue), lesstext):
            painter.drawText(QtCore.QRectF(startpoint, 0, textspace, 20), QtCore.Qt.AlignCenter,
                             str(self.freqValue[i]))#x-Achse Beschriftung
            startpoint = startpoint + textspace*lesstext
    ...
    def draw_ticks(self, balkenbreite, painter):
        ...
        for b in range(0, anzahl_striche):
            painter.drawLine(QtCore.QLineF(-5, y_axis, 0, y_axis)) #y achse einheiten striche
