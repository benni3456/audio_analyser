...
class GainPlotter(QtGui.QWidget):
    """ Plots the gain (input array) of audio signal in dB as a function of time 
    ...
    def draw_text(self, painter):
        """ Function to title and to label the axis of abscissae and the ordinate """
        ...
        for i in range(0, len(self.timestep), 1):
            ...
            startpoint = startpoint + (self.width() - 200) / (len(self.timestep))
        # label ordinate
        y_axis_lines = 5
        y_axis = -10
    ...
    def draw_ticks(self, temporaldist, painter):
        ...
        painter.save()
        painter.scale(((self.width()-200)/(len(self.timestep))),-((self.height()-200)/self.y_range))
    ...
    def draw_data(self, temporaldist, painter):
        ...
        painter.scale(((self.width()-200)/(len(self.timeValue)/
                                           (self.temporaldist))*(1/self.temporaldist)),
        ...
        for a in range(0, len(self.dBValue)-1, 1):
            ...
            painter.drawLine(QtCore.QLineF(startpoint, db, startpoint - 1,db2))
...
#     # gain-values in array
#     dBValue = abs(randn(48000) * 20+10)
#     # gain-values in array
#     timeValue = range(0, 48000)
#     # time-array from 0s to -30s (to plot data of the last 30s)
#     timeStep=range(-30,1)
#     # calculate temporal distance between two sequent values
#     temporalDist = 1/(len(dBValue)/30)
# 
#     app = QtGui.QApplication(sys.argv)
#     dt = GainPlotter()
#     dt.read_array(dBValue, timeValue, timeStep, temporalDist)
#     dt.show()
#     app.exec_()
#===============================================================================
