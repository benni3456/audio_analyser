...
class GainPlotter(QtGui.QWidget):
    ...
    def draw_text(self, painter):
        ...
        painter.drawText(QtCore.QRectF(self.width() - 100, 20, 20, 20),
                            QtCore.Qt.AlignCenter, ' t [s] ')
        painter.drawText(QtCore.QRectF(-30, -self.height() + 70, 50, 20),
                            QtCore.Qt.AlignCenter, ' level [dB] ')
        # label axis of abscissae
        startpoint = 1
        stepsize = (self.width() - 100) / (len(self.timestep))
        for i in range(0, len(self.timestep), 1):
            ...
            startpoint = startpoint + (
                                self.width() - 100) / (len(self.timestep))
        # label ordinate
        y_axis_lines = 5
        y_axis = -10
    ...
    def draw_data(self, temporaldist, painter):
        ...
        painter.scale(((self.width() - 100) / (len(self.timeValue) /
                                           (self.temporaldist))
                                           * (1 / self.temporaldist)),
                                           -((self.height() - 100)
                                             / self.y_range))
        ...
        for a in range(0, len(self.dBValue) - 1, 1):
            ...
            if db2 < -80:
                db2 = -80
            painter.drawLine(QtCore.QLineF(startpoint, 80+db, startpoint + 1,
                                            80+db2))
    ...
    def paintEvent(self, event):
        ...
        painter.translate(50, self.height() - 50)
